...
class author_image_admin {
	...
	function init() {
		...
		if(current_user_can('subscriber') == false) {
	...
	function edit_image() {
        ...
        echo '<tr>'
      		. '<th><label for="sem_aboutme_page">Position</label></th>'
      		. '<td>'
      	    ...
      	    . '<span class="description">Please enter your position' . "'s url.</span>"
      		. '</td>'
      		. '</tr>';
	...
	function save_image($user_ID) {
		...
		if ( isset($_FILES['author_image']['name']) && $_FILES['author_image']['name'] ) {
			...
			if ( !in_array($ext, array('jpg', 'jpeg', 'png')) ) {
				...
				$new_name_sp = WP_CONTENT_DIR . '/authors/' . $author_login . '-' . $entropy . '_sp' . '.' . $ext;
				...
				if ( $width_orig > 0 || $height_orig > 0 ) {
					$scale = 0.243;
					$src_h = $scale * $width_orig;
					$src_y = ($height_orig - $src_h) / 2;
					...
					// Resample
					$image_p = imagecreatetruecolor($width_orig, $src_h);
					...
					imagecopyresampled ( $image_p , $image , 0 , 0 , 0 , $src_y , $width_orig, $src_h, $width_orig, $src_h);
					// imagecopyresampled($image_p, $image, 0, 0, 0, 0, $width, $height, $width_orig, $height_orig);
					
					imagejpeg($image_p, $new_name, 90);

					move_uploaded_file($tmp_name, $new_name_sp);
				} else {
