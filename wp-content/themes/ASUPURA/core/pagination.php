...
class Pagination
{
    ...
    {   
        ...
        if ($this->_config['total_record'] > $this->_config['limit'])
        {
            ...
            if ($this->_config['current_page'] > 1)
            {
                $p .= '<li id="pagi-page-next"><a href="'.$this->__link($this->_config['current_page']-1).'">&laquo;</a></li>';
            }


            if($this->_config['min'] > 2) {
                $p .= '<li><a href="'.$this->__link('1').'">1</a></li>';
                $p .= '<li><a>...</a></li>';
            }

            if($this->_config['min'] == 2) {
                 $p .= '<li><a href="'.$this->__link('1').'">1</a></li>';
                 $this->_config['max'] = $this->_config['max']-1;
            }
            ...
            if($this->_config['max'] < ($this->_config['total_page'] - 1)) {
                $p .= '<li><a>...</a></li>';
                $p .= '<li><a href="'.$this->__link($this->_config['total_page']).'">'.$this->_config['total_page'].'</a></li>';
            }
            ...
            if ($this->_config['current_page'] < $this->_config['total_page'])
            {
                $p .= '<li id="pagi-page-next"><a href="'.$this->__link($this->_config['current_page'] + 1).'">&raquo;</a></li>';
            }
    ...
    {   
        $p = '';
        if ($this->_config['total_record'] > $this->_config['limit']) {
            $p = '<div class="event-pagination box-page-sp">';
                if($this->_config['current_page'] == 1) {
                    $p .= '<div class="event-pagination-btn1"><a href="'.$this->__link(1).'" class="sp-btn-pagination"><img src="'.site_url().'/wp-content/themes/ASUPURA/images/event_pagination_pre.png" alt="">前へ</a></div>';
                } else {
                    $p .= '<div class="event-pagination-btn1"><a href="'.$this->__link($this->_config['current_page']-1).'" class="sp-btn-pagination"><img src="'.site_url().'/wp-content/themes/ASUPURA/images/event_pagination_pre.png" alt="">前へ</a></div>';
                }            
                $p .= '<div class="event-pagination-btn2"><a href="'.$this->__link($this->_config['current_page'] + 1).'" class="sp-btn-pagination">次へ<img src="'.site_url().'/wp-content/themes/ASUPURA/images/event_pagination_next.png" alt=""></a></div>';
            $p .= '</div>';
        }
        return $p;
    }
